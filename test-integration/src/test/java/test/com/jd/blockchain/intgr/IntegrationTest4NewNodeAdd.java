...
//
    ...
    private WebResponse startNewPeerAndActive(HashDigest ledgerHash, String dbTypeRocksdb, NewParticipant newParticipant1, int id) throws InterruptedException {
        WebResponse webResponse;
        // 启动一个新的参与方，此时只启动HTTP服务，共识服务未开启
        startNewPeerNode(ledgerHash, DB_TYPE_ROCKSDB, newParticipant1, id);
        System.out.println("---------- New Node Start Http But Without Consensus Completed ----------");

        // 激活新参与方的共识状态，更新原有共识网络的视图ID，启动新的参与方共识
        Thread.sleep(5000);
        webResponse = activePartiNode(newParticipant1, ledgerHash);
        System.out.println("---------- Active New Node And View Update Completed ----------");
